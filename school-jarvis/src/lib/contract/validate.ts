...
const PERSON_IDS: CoffeePersonId[] = ["levi", "birgit", "heidi"];

export type ValidateSummaryResult =
  | { ok: true; summary: SchoolJarvisDailySummary }
  | { ok: false; error: string };

function isPersonId(value: unknown): value is CoffeePersonId {
  return typeof value === "string" && PERSON_IDS.includes(value as CoffeePersonId);
}
...
function validateLearning(
  ...
  if (
    typeof o.dueFlashcards !== "number" ||
    !Number.isFinite(o.dueFlashcards) ||
    o.dueFlashcards < 0
  ) {
...
/** Response must only contain contract fields — no docs, history, internals. */
export const CONTRACT_TOP_LEVEL_KEYS = [
  "available",
  "personId",
  "focusDate",
  "nextExam",
...
export function assertNoSensitiveLeak(summary: SchoolJarvisDailySummary): string[] {
  const leaks: string[] = [];
  ...
  for (const needle of [
    ...
    if (json.includes(needle)) leaks.push(needle);
  }
  const keys = Object.keys(summary).sort();
  for (const key of keys) {
    if (!(CONTRACT_TOP_LEVEL_KEYS as readonly string[]).includes(key)) {
      leaks.push(`extra_key:${key}`);
    }
  }
  return leaks;
}
