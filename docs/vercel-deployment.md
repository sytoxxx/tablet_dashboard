...
- Private gate: HttpOnly session cookie + server-side access code check (`/zugang`).
...
- Do **not** put `GH_TOKEN` (or any GitHub deploy token) into Vercel **app** environment variables for Coffee Morning.

## Connect GitHub → Vercel

1. Push feature branch `cursor/vercel-ready-c828` to `sytoxxx/tablet_dashboard` (when ready).
...
### Bus / TRIAS configuration (server-only)

| Name | Required | Secret? | Notes |
|------|----------|---------|-------|
| `BUS_PROVIDER` | Recommended | No | Use `auto` |
| `VERBUND_STEIERMARK_TRIAS_URL` | For live TRIAS | No (public OGD URL) | `http://ogdtrias.verbundlinie.at:8183/stv/trias` |
| `VERBUND_STEIERMARK_REQUESTOR_REF` | Optional | Treat custom values as private | Default `OpenService` |
| `VAO_BASE_URL` | Optional | No | Provider-issued base URL |
| `WEATHER_PROVIDER` | Optional | No | e.g. `open-meteo` |
| `OPENAI_PLAN_MODEL` / `OPENAI_JARVIS_MODEL` | Optional | No | Model ids |
| `SCHOOL_JARVIS_BASE_URL` / `SCHOOL_JARVIS_*` | Optional | Token stays secret | Integration knobs |

### Do not put in Git, client bundles, or Vercel as public vars

- Real access codes, API keys, tokens (including `GH_TOKEN`)
...
- [ ] Feature branch pushed (no secrets in tree)
- [ ] All required env vars set in Vercel
- [ ] `COFFEE_MORNING_ACCESS_CODE` set to a strong private value
- [ ] `BUS_PROVIDER=auto` and TRIAS URL set for live bus
